...
rem r103 light soak: armed sysaudit + probe churn; every iteration kills
...
set OUT=%GUEST_DESKTOP%\r103_soak.txt
...
"%VMRUN%" -T ws -gu %VM_USER% -gp %VM_PASS% CopyFileFromGuestToHost "%VM_VMX%" "%OUT%" "%WINDBG_TEST%\logs\r103_soak.txt"
exit /b %ERRORLEVEL%
